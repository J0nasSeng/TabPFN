...
from tabpfn.hyperpc import HyperPC
...
def train(priordataloader_class, criterion, encoder_generator, emsize=200, nhid=200, nlayers=6, nhead=2, dropout=0.0,
    ...
    n_out = 381 # number of parameters of PC. TODO: Make this adaptive

    model = HyperPC(encoder, n_out, emsize, nhead, nhid, nlayers, dl.num_features, dropout, style_encoder=style_encoder,
        ...
        for batch, (data, targets, single_eval_pos) in enumerate(dl):
            ...
            with cm:
                ...
                with autocast(enabled=scaler is not None):
                    ...
                    lls = model(tuple(e.to(device) if torch.is_tensor(e) else e for e in data) if isinstance(data, tuple) else data.to(device)
                    ...
                    total_loss += lls.mean().detach()

                step_time = time.time() - before_forward


            before_get_batch = time.time()
        return total_loss / steps_per_epoch, time_to_get_batch, forward_time, step_time
    ...
    try:
        for epoch in (range(1, epochs + 1) if epochs is not None else itertools.count(1)):
            ...
            total_loss, time_to_get_batch, forward_time, step_time = train_epoch()
