...
from simple_einet.abstract_layers import AbstractSumLayer
from simple_einet.layers.distributions.abstract_leaf import AbstractLeaf



class HyperPC(nn.Module):
    def __init__(self, encoder, n_out, ninp, nhead, nhid, nlayers, n_features, dropout=0.0, style_encoder=None,
    ...
    def forward(self, src, src_mask=None, single_eval_pos=None):
        ...
        embedding = self.transformer_encoder(src, src_mask)
        weights = self.decoder(embedding)
        self.set_pc_parameters(weights)

        test_x = x_src[single_eval_pos:]
        lls = self.einet(test_x)
    ...
    def set_pc_parameters(self, parameters):
        """
            Set the parameters of the Einet.
        """
        param_start_idx, param_end_idx = 0, 0
        for layer in self.einet.layers:
            num_params = np.prod(layer.weight_shape())
            param_end_idx += num_params

            weights = parameters[param_start_idx:param_end_idx]
            weights = weights.reshape(layer.weight_shape())
            layer.logits = weights

        leaf_params = parameters[param_end_idx:]
        leaf_params = leaf_params.reshape(2, -1)
        mean, std = leaf_params[0,:], leaf_params[1,:]
        mean_shape = self.einet.leaf.base_leaf.means.shape
        std_shape = self.einet.leaf.base_leaf.stds.shape
        self.einet.leaf.base_leaf.means = mean.reshape(mean_shape)
        self.einet.leaf.base_leaf.stds = std.reshape(std_shape)
