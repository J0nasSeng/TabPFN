...
class EinsumLayer(AbstractSumLayer):
    ...
    def _condition_weights_on_evidence(self, ctx: SamplingContext, log_weights: Tensor) -> Tensor:
        ...
        # compute product of likelihoods from left and right side for all products (log-space -> summation)
        lls = (lls_left + lls_right).view(ctx.num_samples, self.num_features_out, self.num_sums_in**2)
        log_prior = log_weights
        # weight each product by corresponding mixture weight (log-space -> summation)
        log_posterior = log_prior + lls
        # adjust mixture weights using logsumexp-trick (this is softmax in log-space)
        # the adjustment is done by marginalizing out all non-evidence variables in each mixture,
        # the logsumexp-trick then normalizes according to the mass provided by each mixture component
        # under the given evidence.
