...
class Einet(nn.Module):
    ...
    def __init__(self, config: EinetConfig):
        ...
        # Leaf cache
        self._leaf_cache = {}
        self.shift_parameters = None

    def init_shift_parameters(self):
        self.shift_parameters = nn.Parameter(torch.zeros((self.config.num_channels, self.config.num_features)))
    ...
    def forward(self, x: torch.Tensor, marginalized_scopes: torch.Tensor = None, cache_index: Optional[int] = None) -> torch.Tensor:
        ...
        if cache_index is not None and cache_index in self._leaf_cache:
            ...
            if self.shift_parameters is not None:
                x = self.leaf(x + self.shift_parameters, marginalized_scopes)
            else:
